const fs = require('node:fs'); // node: prefix for core modules
...
// Utils object with modern property syntax
const Utils = {
  commands: new Map(),
  handleEvent: new Map(),
  account: new Map(),
...
// Directory reading with modern error handling
for (const file of fs.readdirSync(script)) {
  const scripts = path.join(script, file);
  const stats = fs.statSync(scripts);
  
  if (stats.isDirectory()) {
    for (const subFile of fs.readdirSync(scripts)) {
      try {
        const { config, run, handleEvent } = require(path.join(scripts, subFile));
        if (!config) continue;
...
// Server startup
app.listen(3000, () => {
  console.log(`Server is running at http://localhost:3000`); // Fixed port number
});
  ...
  return new Promise((resolve, reject) => {
    login({ appState: state }, async (error, api) => {
      ...
      try {
        ...
        if (!userInfo?.[userid]?.name || !userInfo?.[userid]?.profileUrl || !userInfo?.[userid]?.thumbSrc) {
        ...
        const time = history.find(user => user.userid === userid)?.time || 0;
        ...
        const listenEmitter = api.listenMqtt(async (error, event) => {
          ...
          let hasPrefix = (event.body && aliases((event.body || '')?.trim().toLowerCase().split(/ +/).shift())?.hasPrefix == false) 
            ? '' 
            : prefix;
...
async function deleteThisUser(userid) {
  ...
  try {
    ...
    console.log('Error deleting session file:', error.message);
  }
}
...
function aliases(command) {
  for (const [commands, data] of Utils.commands.entries()) {
...
async function createThread(threadID, api) {
  try {
    ...
    database.push({ [threadID]: adminIDs });
